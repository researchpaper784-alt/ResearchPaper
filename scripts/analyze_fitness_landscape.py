...
def margin(num_clients: int, noise: float, seed: int, gamma_entropy: float) -> float:
    gram = precompute_gram(synthetic_deltas(num_clients, noise, seed))
    return corner_margin(
        gram,
        torch.full((num_clients,), 1.0 / num_clients),
        DataFreeFitnessConfig(gamma_entropy=gamma_entropy),
    )


def mean_margin(num_clients: int, noise: float, gamma_entropy: float) -> float:
    return statistics.fmean(margin(num_clients, noise, s, gamma_entropy) for s in SEEDS)


def required_gamma_entropy(num_clients: int, noise: float) -> float:
    ...
    for _ in range(40):
        mid = (low + high) / 2
        if mean_margin(num_clients, noise, mid) > 0:
            low = mid
        else:
            high = mid
    return high
